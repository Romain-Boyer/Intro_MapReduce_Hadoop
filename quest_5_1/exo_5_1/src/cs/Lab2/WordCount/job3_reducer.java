...
public class job3_reducer extends Reducer<Text, Text, Text, Text> {
	...
	protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		...
		for (String document : tempFrequencies.keySet()){
			...
			// inverse document frequency quotient between the nuber of docs in corpus and number of docs the term appears
			...
			context.write(new Text(key + "@" + document), new Text("[" + numberOfDocumentsInCorpusWhereKeyAppears + "/"+ numberOfDocumentsInCorpus + " , " + wordFrequenceAndTotalWords[0] + "/" + wordFrequenceAndTotalWords[1] + " , " + DF.format(tfIdf) + "]"));
		}
